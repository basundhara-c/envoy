...
public:
  ...
  /**
   * Enumerates the names of live config objects: the active listeners' filter chains, the clusters
   * and their transport socket matches, and the active dynamic TLS certificate secrets. `emit` is
   * invoked once per name with its kind. A no-op before the server is initialized. Main thread only.
   *
   * @param emit is invoked for each name; the string_view is valid only during that invocation.
   */
  void iterateConfigNames(
      absl::FunctionRef<void(envoy_dynamic_module_type_bootstrap_config_name_kind, absl::string_view)>
          emit);

  /**
   * Returns the transport socket match names present in every cluster that has matches. A match is
   * observed only once it is in all such clusters; clusters with no matches do not constrain the
   * result. Exposed for testing.
   */
